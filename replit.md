...
*   **One-Component-Per-Round Rule**: Enforces tactical variety by preventing any material component from being used more than once within a single round across all spells. **Exception**: Containers (Air Sphere, Vortex) and Gust (propulsion) are always available and exempt from this rule, allowing multiple offensive spells per round. Visual feedback includes grayed-out materials (opacity-40, grayscale filter, cursor-not-allowed) in the Component Library and disabled dragging (`draggable=false`). Attempting to reuse a material shows a toast notification. Backend validation in `validateSpell()` recursively checks for duplicate `baseId` values but skips containers and propulsion. AI spell generation respects this rule via `getComponentIds()` tracking. Components reset to available state when a new round begins after clicking "Next Round" in the Results Modal. Each component card has `data-is-used` and `data-component-id` attributes for testing verification.
*   **Frontend/Backend Damage Consistency**: Damage calculations are synchronized between frontend (Spell Breakdown display) and backend (combat resolution). Specialization bonus (+20%) is applied per-container. Intellect bonus (`floor(intellect/2)`) is calculated identically on both sides and applied once to the first damage spell in multi-spell scenarios.
